...
AnalogIn::AnalogIn(pin_name pin)
{
...
void AnalogIn::configPin()
{
  ...
  if((portNum == PORTA) && pinNum <= PTA7) ADC1->CHSELR |= (1ul << pinNum); //PTA0 - PTA7
  else  if((portNum == PORTB) && pinNum <= PTB1) ADC1->CHSELR |= (1ul << (pinNum + 8)); //PB0 - PB1
  else if((portNum == PORTC) && pinNum <= PTC5)  ADC1->CHSELR |= (1ul << (pinNum + 10));  //PTC0 - PTC5
  else 
  {
    RCC->APB2ENR &= ~RCC_APB2ENR_ADC1EN;
    return;
